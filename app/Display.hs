...
prettyFlat :: T.Text -> [FlatStep] -> T.Text
prettyFlat name steps =
  name
    <> ":\n\n"
    <> ( T.intercalate "\n" $
           zipWith4
...
prettyFitch :: FitchProof -> T.Text
prettyFitch (FitchProof name _ prems steps) =
  name
    <> ":\n\n"
    <> ( T.intercalate "\n" $
           zipWith3
  ...
  where
    ...
    prettyStep indnt (FitchStep wff rule cites) =
      let bars = T.replicate indnt "│ "
       in [ (True, bars <> render (prettyWff wff), rule <> " " <> printCites cites)
          ]
    prettyStep indnt (FitchSubproof assump substeps) =
      let bars = T.replicate indnt "│ "
          expr = render $ prettyWff assump
          assumpLines =
